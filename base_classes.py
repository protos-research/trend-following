...
class Trend_Data(Data):
    ...
    def load_data(self):
        tickers = ['Date','bitcoin','bitcoin-cash','ethereum','litecoin','ripple']
        scope = ['https://spreadsheets.google.com/feeds',
                 'https://www.googleapis.com/auth/drive']
        creds = ServiceAccountCredentials.from_json_keyfile_name('AWSLambda-f3f1c3a0c139.json', scope)
        client = gs.authorize(creds)
        ss_price = client.open("Backtesting Data").worksheet('Price')
        df_price = get_as_dataframe(ss_price)
        df_price_tickers = df_price[tickers] #self.tickers
